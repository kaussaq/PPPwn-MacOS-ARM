...
Clone this repository to your mac system, then from the repo folder run `./build-macarm.sh`. This will build the binaries for PS4 FW 1100 and place the necessary files into the correct folders. To build the binaries for a different version, e.g. FW 9.00, run the command as such: `./build-macarm.sh 900`.

The enhanced build script will automatically:
- Install Homebrew if not present
- Install Docker Desktop if not present  
- Start Docker Desktop and wait for it to be ready
- Build the PPPwn binaries using Docker
- Verify the build was successful and display file locations

Once built, you can either:
- Copy this folder structure into a Linux VM and execute as instructed above
- Run the exploit directly on macOS using the same commands as Linux (Python3 and network interface setup required)
